...
export async function gerarImagem(orc, linhas, tot, ds) {
  ...
  medir.font = `700 28px ${F}`;
  const linhasCliente = quebrar(medir, orc.cliente || '—', W / 2 - M - 20);
  const extraCliente = (linhasCliente.length - 1) * 34;
  const H = 300 + altItens + 260 + (tot.st ? 44 : 0) + (docFmt(orc.clienteDoc) ? 30 : 0) + extraCliente;
  ...
  linhasCliente.forEach((t, j) => ctx.fillText(t, M, y + j * 34));
  y += extraCliente;
...
export async function gerarPdf(orc, linhas, tot, ds) {
  ...
  doc.setFontSize(11);
  doc.setTextColor(TINTA);
  const nomeCliente = doc.splitTextToSize(ansi(orc.cliente || '—'), W / 2 - M - 6);
  doc.text(nomeCliente, M, 43, { lineHeightFactor: 1.15 });
  doc.text(ansi(orc.vendedor), W / 2, 43);
  let yCab = 43 + (nomeCliente.length - 1) * 5;
  if (docFmt(orc.clienteDoc)) {
    yCab += 5;
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9);
    doc.setTextColor(CORPO);
    doc.text(docFmt(orc.clienteDoc), M, yCab);
  }
